...
#' \eqn{\code{ncol(Xnew)}\times 1} vector of the point predictions; otherwise, the object is an
#' \eqn{\code{ncol(Xnew)}\times 3} matrix with the point predictions in the first column and the prediction intervals
...
#' Z.interval <- predict(object = res, model = "MAP", Xnew = newx, interval = "Z", level = 0.95)
#' @method predict sven
#' @export

predict.sven <- function(object, Xnew, model = c("WAM", "MAP"), interval = c("none", "MC", "Z"),
                         return.draws = FALSE, Nsim = 10000, level = 0.95, alpha = 1-level) {
  y.pred <- predPoint(object = object, newx = Xnew, model = model)
  if (interval == "none"){
    return(y.pred)
  } else {
    intv <- predIntv(object = object, model = model, Xnew = Xnew, interval = interval, Nsim = Nsim,
