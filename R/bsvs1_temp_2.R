bsvs1_temp <- function(xmat, ys, xty, lam, w, topKeep, D, xbar, temp, temp.multip, logp.best, r.idx.best, n, ncovar, Miter) {
  ...
  rc.k <- res1[rc.k.idx]
  s <- sample.int(k, 1, prob = exp((rc.k-max(rc.k, na.rm = T))/(temp*temp.multip)))
