...
sven.xz <- function(x1,x2, y, w1 = sqrt(nrow(x1))/ncol(x1),w2 = sqrt(nrow(x2))/ncol(x2),
  ...
  for(i in 1:length(cols.top)) {
    ...
    if (size.top[i]==0){ 
      ...
      m_i = model.top[, i] 
      m1_i =which(m_i[1:p1]); m2_i=which(m_i[(p1+1):(p1+p2)]) 
      u.est <- cbind(rep(1, n), scale(x1[, m1_i], center = x1bar[m1_i], scale = 1/D1[m1_i]), scale(x2[, m2_i], center = x2bar[m2_i], scale = 1/D2[m2_i]))
      pg1 = length(m1_i) ; pg2 = length(m2_i) 
      A1 <- lam1*diag(pg1)
      A2 <- lam2*diag(pg2)
      A <- bdiag(0,A1,A2) 
      beta <- solve(crossprod(u.est) + A , crossprod(u.est, y)) 
      meanvec <- as.numeric(c(x1bar[m1_i],x2bar[m2_i]))
      diagmat <-as.numeric(c(D1[m1_i],D2[m2_i]))
      beta.est[c(T,m_i), i] <- c(beta[1]-sum(beta[-1]*meanvec*diagmat), beta[-1] * diagmat)
    } 
  }
